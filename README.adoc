...
Although only2 contributors, i picked it because of its nice examples that fit
well.
This time, none of the standard examples worked out, so i had to dive deeper
into the library.

=== beefsack's go-aster library
...
But this interface-only pattern leads to a problem.
In path finding problems, you usually have an area, often a grid, and cells.
...
ERROR: panic() somewhere deep inside the guts of the external library.
